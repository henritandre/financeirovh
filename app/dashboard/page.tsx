...
export default function DashboardPage() {
  ...
  const [data, setData] = useState(getDatLocalHoje());
  ...
  // Estado para guardar TODOS os usuários que têm lançamentos e os SELECIONADOS
  ...
  const carregarDados = async (isInitialLoad = false) => {
    ...
    if (historico) {
      ...
      // MÁGICA DOS USUÁRIOS: Descobre todos os nomes únicos que existem no banco
      const unicos = Array.from(new Set(historico.map(t => t.autor_nome || "Usuário")));
      setUsuariosDisponiveis(unicos);
      
      // Se for a primeira vez carregando, já deixa todas as pílulas "acesas"
      if (isInitialLoad) {
  ...
  useEffect(() => {
    const loadInit = async () => {
      ...
      if (user) {
        ...
        carregarDados(true); // "true" avisa que é o primeiro carregamento
      } else {
  ...
  const transacoesFiltradas = transacoes.filter((t) => {
    ...
    // O lançamento só aparece se o autor estiver na lista de pílulas ativas
  ...
  // Função que liga/desliga a pílula do usuário
  const toggleUsuario = (nome: string) => {
    setUsuariosSelecionados(prev => 
      prev.includes(nome) 
        ? prev.filter(u => u !== nome) // Desmarca
        : [...prev, nome] // Marca
    );
  };
  ...
  const handleSalvarTransacao = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("transacoes").insert([{ 
      ...
      descricao, valor: valorNumerico, data, tipo, categoria_id: categoriaId 
    }]);
    
    setIsSubmitting(false);
    if (error) { showToast("Erro ao salvar: " + error.message, "error"); } 
    else { setDescricao(""); setValor(""); setCategoriaId(""); setIsModalOpen(false); showToast("Lançamento salvo!"); carregarDados(false); }
  };

  const abrirModalDeExclusao = async (transacao: any) => {
    if (transacao.user_id !== userId) { showToast("Você só pode excluir os lançamentos criados por você.", "error"); return; }
    ...
    if (excluidas && excluidas.length > 0) {
      const contagem: Record<string, number> = {}; excluidas.forEach(t => { contagem[t.motivo] = (contagem[t.motivo] || 0) + 1; });
      const top3 = Object.entries(contagem).sort((a, b) => b[1] - a[1]).slice(0, 3).map(item => item[0]); setMotivosFrequentes(top3);
  ...
  const confirmarExclusaoComAuditoria = async (e: React.FormEvent) => {
    ...
    setIsDeleting(true);
    const { error: erroInsert } = await supabase.from("transacoes_excluidas").insert([{ 
      transacao_id: transacaoParaApagar.id, descricao: transacaoParaApagar.descricao, valor: transacaoParaApagar.valor, data: transacaoParaApagar.data, tipo: transacaoParaApagar.tipo, categoria_id: transacaoParaApagar.categoria_id, user_id: transacaoParaApagar.user_id, autor_nome: transacaoParaApagar.autor_nome, excluido_por_nome: username || "Usuário", motivo: motivoExclusao 
    }]);
    ...
    setIsDeleting(false);
    if (erroDelete) { showToast("Erro ao apagar.", "error"); } else { showToast("Lançamento arquivado!"); setIsDeleteModalOpen(false); carregarDados(false); }
  };
    ...
    <div className="min-h-screen bg-gray-50 relative pb-20 overflow-x-hidden">
              ...
              <>
                ...
                <div className="absolute right-0 top-full mt-3 w-56 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50">
                  <div className="p-2 space-y-1">
                    ...
                    <button onClick={() => router.push("/auditoria")} className="w-full text-left px-4 py-2.5 text-sm font-bold text-gray-700 hover:bg-gray-50 hover:text-blue-600 rounded-lg transition-colors">Lançamentos Excluídos</button>
      ...
      <main className="p-6 max-w-6xl mx-auto space-y-6 mt-4 relative z-0">
        <div className="flex justify-between items-end">
          ...
          <button onClick={() => setIsModalOpen(true)} className="hidden md:flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-sm active:scale-95"><span className="text-xl leading-none">+</span> Novo Lançamento</button>
        </div>
        ...
        {/* --- BARRA DE FILTROS COM AS PÍLULAS --- */}
        <div className="bg-white p-4 sm:p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 mt-8">
          
          <div className="flex gap-4 flex-col sm:flex-row flex-1">
            <div className="flex-1">
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5 ml-1">Mês</label>
              <input type="month" value={filtroMes} onChange={(e) => setFiltroMes(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" />
            </div>
            <div className="flex-1">
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5 ml-1">Tipo</label>
              <select value={filtroTipo} onChange={(e) => setFiltroTipo(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all cursor-pointer">
          ...
          {/* AS PÍLULAS MÁGICAS DE USUÁRIOS */}
          <div className="flex-1">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-2 ml-1">Filtrar por Usuário</label>
            <div className="flex flex-wrap gap-2">
              {usuariosDisponiveis.length === 0 ? (
                <span className="text-sm text-gray-400 font-medium py-2">Buscando usuários...</span>
              ) : (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mt-2">
          ...
          {transacoesFiltradas.length === 0 ? (
             <div className="p-10 flex flex-col items-center justify-center text-center gap-3"><p className="text-gray-500 font-bold">Nenhum lançamento encontrado para estes filtros.</p></div>
          ) : (
                  ...
                  <div className="flex items-center gap-4 overflow-hidden">
                    ...
                    <div className="truncate">
                      ...
                      <div className="flex items-center gap-2 mt-1 text-xs font-bold text-gray-500">
                        ...
                        <span>•</span><span>{formatarData(t.data)}</span>
                        <span>•</span><span className="text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md">@{t.autor_nome || 'Usuário'}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className={`text-base sm:text-lg font-black ${t.tipo === 'receita' ? 'text-green-600' : 'text-gray-900'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50/80">
              <h3 className="text-lg font-black text-gray-900">Novo Lançamento</h3><button onClick={() => setIsModalOpen(false)} className="text-gray-500 hover:text-gray-900 text-2xl font-bold">&times;</button>
            </div>
            <form onSubmit={handleSalvarTransacao} className="p-6 space-y-5">
              <div className="flex p-1 bg-gray-200 rounded-xl">
                <button type="button" onClick={() => { setTipo("despesa"); setCategoriaId(""); }} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${tipo === "despesa" ? "bg-white text-red-600 shadow-sm" : "text-gray-600 hover:text-gray-800"}`}>Despesa</button>
                <button type="button" onClick={() => { setTipo("receita"); setCategoriaId(""); }} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${tipo === "receita" ? "bg-white text-green-600 shadow-sm" : "text-gray-600 hover:text-gray-800"}`}>Receita</button>
              </div>
              ...
              <button type="submit" disabled={isSubmitting} className={`w-full py-3.5 rounded-xl text-white font-black uppercase tracking-wide transition-all shadow-md active:scale-95 ${tipo === 'receita' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}>
                {isSubmitting ? "Salvando..." : `Salvar ${tipo === 'receita' ? 'Receita' : 'Despesa'}`}
              </button>
            </form>
          </div>
        </div>
      ...
      {isDeleteModalOpen && transacaoParaApagar && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-0">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 border border-red-100">
            <div className="px-6 py-5 border-b border-gray-100 bg-red-50/50 flex justify-between items-center"><h3 className="text-lg font-bold text-red-600 flex items-center gap-2">Apagar Lançamento</h3><button onClick={() => setIsDeleteModalOpen(false)} className="text-gray-400 hover:text-gray-600 text-2xl">&times;</button></div>
            <form onSubmit={confirmarExclusaoComAuditoria} className="p-6 space-y-5">
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-200"><p className="text-sm text-gray-500 font-bold">Você está prestes a excluir:</p><p className="text-lg font-black text-gray-900 mt-1 truncate">{transacaoParaApagar.descricao}</p><p className={`text-base font-black ${transacaoParaApagar.tipo === 'receita' ? 'text-green-600' : 'text-red-600'}`}>{formatarMoeda(transacaoParaApagar.valor)}</p></div>
              <div><label className="block text-sm font-bold text-gray-700 mb-2">Motivo da Exclusão</label><textarea required rows={2} value={motivoExclusao} onChange={(e) => setMotivoExclusao(e.target.value)} placeholder="Por que está apagando isso?" className="block w-full rounded-xl border border-gray-300 bg-white p-3 text-sm font-bold text-gray-900 focus:border-red-500 focus:ring-4 focus:ring-red-500/20 resize-none" /></div>
              {motivosFrequentes.length > 0 && (
                <div className="flex flex-wrap gap-2"><span className="text-xs font-bold text-gray-600 w-full mb-1">Motivos recentes:</span>{motivosFrequentes.map((motivo, idx) => (<button key={idx} type="button" onClick={() => setMotivoExclusao(motivo)} className="text-xs font-bold bg-gray-200 hover:bg-gray-300 text-gray-800 py-1.5 px-3 rounded-full transition-colors">{motivo}</button>))}</div>
              )}
              <div><label className="block text-sm font-bold text-gray-700 mb-2">Digite <span className="text-red-600 font-black uppercase">EXCLUIR</span> para confirmar</label><input type="text" required value={palavraConfirmacao} onChange={(e) => setPalavraConfirmacao(e.target.value)} placeholder="excluir" className="block w-full rounded-xl border border-gray-300 bg-white p-3 text-sm font-black text-gray-900 focus:border-red-500 focus:ring-4 focus:ring-red-500/20 text-center tracking-widest uppercase" /></div>
              <button type="submit" disabled={isDeleting || palavraConfirmacao.toLowerCase() !== "excluir" || motivoExclusao.trim() === ""} className="w-full py-3.5 rounded-xl text-white font-black transition-all active:scale-95 bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed">{isDeleting ? "Apagando..." : "Confirmar Exclusão"}</button>
            </form>
          </div>
        </div>
      )}
