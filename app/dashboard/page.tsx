...
export default function DashboardPage() {
  ...
  // ==========================================
  // ESTADOS GERAIS DO USUÁRIO E VISIBILIDADE
  // ==========================================
  ...
  const carregarDados = async (isInitialLoad = false) => {
    ...
    if (error) { showToast("Erro ao carregar dados.", "error"); }
  ...
  const transacoesFiltradas = transacoes.filter((t) => {
    ...
    const bancoOk = filtroBanco === "" || 
                    t.conta_origem?.banco_vinculado?.id === filtroBanco || 
                    t.conta_destino?.banco_vinculado?.id === filtroBanco;
    const contaOk = filtroFormaPagto === "" || 
                    t.conta_id === filtroFormaPagto || 
                    t.conta_destino_id === filtroFormaPagto;
  ...
  // ==========================================
  // MATEMÁTICA BANCÁRIA E ORDENAÇÃO
  // ==========================================
  ...
  const abrirModalEditar = (t: any) => {
    if (t.user_id !== userId) { showToast("Apenas o autor pode editar este lançamento.", "error"); return; }
    resetFields();
  ...
  const executarSalvarTransacaoBD = async (e?: React.FormEvent) => {
    ...
    if (!dbContaId) { showToast("Erro: Forma de pagamento inválida.", "error"); setIsProcessingAudit(false); return; }
    if (tipo === "transferencia" && !dbContaDestinoId) { showToast("Erro: Conta destino inválida.", "error"); setIsProcessingAudit(false); return; }
    ...
    if (editandoId) {
      ...
      if (error) showToast("Erro: " + error.message, "error"); else showToast("Atualizado!");
    } else {
  ...
  const confirmarExclusaoComAuditoria = async (e: React.FormEvent) => {
    ...
    setIsProcessingAudit(false); showToast("Excluído com sucesso!"); setIsDeleteModalOpen(false); carregarDados(false);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 relative pb-20 overflow-x-hidden transition-colors duration-300">
      ...
      {/* NAVBAR UNIFICADA (AGORA COM OLHINHO GLOBAL) */}
      ...
      <main className="p-6 max-w-6xl mx-auto space-y-6 mt-4 relative z-0">
        ...
        {/* 1. SEÇÃO DE FILTROS */}
        <div className="bg-white dark:bg-gray-800 p-4 sm:p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col space-y-4 transition-colors">
        ...
        {/* 2. SEÇÃO DE SALDOS BANCÁRIOS E COFRES (SALDO REAL) */}
        {!isLoadingData && (
        ...
        {/* 4. ÁREA DE CARTÕES DE CRÉDITO */}
        <div className="bg-purple-50/40 dark:bg-purple-900/10 p-6 sm:p-8 rounded-2xl shadow-sm border border-purple-100 dark:border-purple-900/30 flex flex-col w-full mt-4 transition-colors">
        ...
        {/* 5. LISTAGEM DE TRANSAÇÕES (EXTRATO) */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden mt-2 transition-colors">
